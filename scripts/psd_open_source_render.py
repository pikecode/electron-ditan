...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Replace the PSD base pixel layer with a custom source image while "
            "preserving the original layer style stack, then render via psd-tools."
        )
    )
    ...
    parser.add_argument(
        "--reference",
        help="Optional reference image path for mean absolute diff metrics",
    )
    parser.add_argument(
        "--compare-against",
        action="append",
        default=[],
        help="Optional extra images to compare against",
    )
    parser.add_argument(
        "--metrics-json",
        help="Optional path to save render metadata and diff metrics as JSON",
    )
    parser.add_argument(
        "--source-preprocess",
...
def probe_pyvips() -> dict:
    ...
    try:
        ...
        return {
            "available": True,
            "version": getattr(pyvips, "__version__", ""),
        }
...
def transplant_source_with_effects(
    psd: PSDImage,
    source_path: str,
    ...
    if edge_source_path:
        ...
        placed, edge_source_applied = blend_edge_safe_source(placed, edge_placed, alpha_reference_layer)
    ...
    new_layer._record.tagged_blocks = deepcopy(old_layer._record.tagged_blocks)

    psd.remove(old_layer)
    psd.insert(replacement_layer_index, new_layer)
    rendered = psd.composite(apply_icc=True)
    ...
    metadata = {
        ...
        "replacement_layer_edge_source_applied": edge_source_applied,
...
def compute_diff_metrics(rendered: Image.Image, reference_path: str) -> dict:
    reference = Image.open(reference_path).convert("RGB")
    if reference.size != rendered.size:
        raise ValueError(
            f"Reference size mismatch: rendered={rendered.size}, reference={reference.size}"
        )
    rendered_np = np.asarray(rendered.convert("RGB")).astype(np.int16)
    reference_np = np.asarray(reference).astype(np.int16)
    diff = np.abs(rendered_np - reference_np)
    return {
        "path": reference_path,
        "mean_abs_diff": diff.mean(axis=(0, 1)).round(3).tolist(),
        "render_mean_rgb": rendered_np.mean(axis=(0, 1)).round(3).tolist(),
        "reference_mean_rgb": reference_np.mean(axis=(0, 1)).round(3).tolist(),
    }
...
def main() -> None:
    ...
    rendered, metadata = transplant_source_with_effects(
        psd,
        args.source,
        args.edge_source,
        args.fit,
    ...
    if args.reference:
        metrics["comparisons"].append(compute_diff_metrics(rendered, args.reference))
    for extra in args.compare_against:
        metrics["comparisons"].append(compute_diff_metrics(rendered, extra))
