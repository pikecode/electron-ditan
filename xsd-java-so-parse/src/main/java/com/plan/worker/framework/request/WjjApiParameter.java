...
public class WjjApiParameter extends HashMap<String, Object> {
    ...
    public WjjApiParameter() {

    }

    public WjjApiParameter(String key, Object value) {
        this.put(key, value);
    }
    ...
    public WjjApiParameter getJsonObject(String key, boolean jsonStr) {
        if (this.containsKey(key)) {
            String json = this.getString(key, "{}");
            if (jsonStr) {
                json = JSONObject.toJSONString(this.get(key));
            }
            try {
